...
class VGMediaViewController: UIViewController {
    ...
    var player = VGPlayer()
    var urlString = ""
    var url1  = URL(string: "http://lxdqncdn.miaopai.com/stream/6IqHc-OnSMBIt-LQjPJjmA__.mp4?ssig=a81b90fdeca58e8ea15c892a49bce53f&time_stamp=1508166491488")!
...
//        print(urlString)
//        url1 = URL(string: urlString) as! URL
        
        let temp = URL(string: urlString.toSecureHTTPSofDomainChange().addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)!)
        if (urlString != "") {
            
            self.player.replaceVideo(temp!)
        } else {
            
            self.player.replaceVideo(self.url1)
        }
